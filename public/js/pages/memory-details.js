...
export function renderMemoryDetailsPage() {
  return `
    <div class="container">
      <header>
        <div class="logo">
      ...
      ${renderDropdown('memory-breakdown', 'Memory Breakdown', `
        <div class="mt-1">
      ...
      ${renderDropdown('memory-history', 'Memory Usage History', `
...
export function updateMemoryDetails(data) {
  ...
  const breakdown = document.querySelector('[data-dropdown-content="memory-breakdown"]');
  if (breakdown) {
    const rows = breakdown.querySelectorAll('.info-row');
    if (rows.length >= 6) {
      if (rows[0].querySelector('.info-value')) {
        rows[0].querySelector('.info-value').textContent = formatBytes(mem.total);
      }
      if (rows[1].querySelector('.info-value')) {
        rows[1].querySelector('.info-value').textContent = formatBytes(mem.used);
      }
      if (rows[2].querySelector('.info-value')) {
        rows[2].querySelector('.info-value').textContent = formatBytes(mem.free);
      }
      if (rows[3].querySelector('.info-value')) {
        rows[3].querySelector('.info-value').textContent = usedPercent + '%';
      }
      if (rows[4].querySelector('.info-value')) {
        const cached = mem.total - mem.used - mem.free;
        rows[4].querySelector('.info-value').textContent = cached > 0 ? formatBytes(cached) : 'N/A';
      }
      if (rows[5].querySelector('.info-value')) {
        rows[5].querySelector('.info-value').textContent = formatBytes(mem.free);
      }
    }
  }
  ...
  if (cards.length >= 4) {
    ...
    if (percentExtra) {
      percentExtra.textContent = `${formatBytes(mem.used)} used of ${formatBytes(mem.total)} total`;
    }
  }
...
export function initMemoryDetails() {
  setTimeout(() => {
    initDropdowns();
...
export function getLastMemoryData() {
